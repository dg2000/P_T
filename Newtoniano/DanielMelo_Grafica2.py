...
if(z[len(z)-1] == 0.0):
    ...
    print "La frontera del sistema se encontro en z = " + str(z[len(z)-1]) + ", para n = " + str(y2[len(z)-1]) + "  y " + str(y1[len(z)-1])
...
plt.savefig("w'_vs_z.png")

f = open ('frontera_vs_n.txt','a')
f.write('\n' + str(y2[len(z)-1]) + " " + str(z[len(z)-1]) + " " + str(y1[len(z)-1]))
f.close()
